{
    ...
    //            oldest requests.
    //
    //            The algorithm does not actually use the idle function, it
    //            reacts to button presses on the floors and in the elevators.
    //
    //            Elevators will stop at intermediary floors toward their
    ...
    init: function(elevators, floors) {
        ...
        var hasRequest = function (floorNum, direction) {
            return requests.some(function(r) {
                return r[0] === floorNum && r[1] === direction;
            });
        };
        ...
        var removeRequest = function (floorNum, direction) {
            requests = requests.filter(function (value) {
        ...
        elevators.forEach(function(elevator) {
            ...
            // people on that floor waiting to go in the same direction.
        ...
        var elGoto = function(elevator, floorNum) {
            ...
            if (elevator.destinationQueue.length > 0) {
                ...
                if (nextFloorNum > curFloorNum) {
                    var firstHalf = elevator.destinationQueue.filter(function(value) {
                        return value >= curFloorNum;
                    });
                    firstHalf.sort(function(a, b) { return a - b; });
                    var secondHalf = elevator.destinationQueue.filter(function(value) {
                        return value < curFloorNum;
                    });
        ...
        floors.forEach(function(floor) {
            ...
            // Bug fix: pass direction as first (and only needed) argument.
            // The floorNum is captured via floor.floorNum() inside queueRequest.
            floor.my_onButton = function(direction) {
    ...
    update: function(dt, elevators, floors) {
        // Cancel elevator movements toward floors that were cleared by
